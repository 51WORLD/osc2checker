...
sIBaseUnitSpecifier : 'SI' '(' sIBaseExponentList ')';
...
//[Antlr4 Action: Java version]
sIBaseExponent : sIBaseUnitName = Identifier {
		String unitName = $sIBaseUnitName.text;
...
enumMemberDecl : enumMemberName ('=' numMemberValue )?;

numMemberValue: UintLiteral | HexUintLiteral;
...
structDeclaration : 
	'struct' structName 
	('inherits' structName ('(' fieldName '=='(enumValueReference | BoolLiteral) ')')?)? 
...
actorDeclaration :
	'actor' actorName
	('inherits' actorName ('(' fieldName '==' (enumValueReference | BoolLiteral) ')')? )?
...
// scenarioDeclaration
scenarioDeclaration 
	: 'scenario' qualifiedBehaviorName 
	('inherits' qualifiedBehaviorName ('(' fieldName '==' (enumValueReference | BoolLiteral)')')? ) ?
	((':' NEWLINE INDENT
...
riseExpression : 'rise' '(' boolExpression ')';
fallExpression :'fall' '(' boolExpression ')';
elapsedExpression : 'elapsed' '(' durationExpression ')';
//[Antlr4 Action: Java version]
everyExpression : 'every' '(' durationExpression (',' Identifier{ 
		String offsetName = $Identifier.text;
...
parameterWithMember : constraintDeclaration;
...
removeDefaultDeclaration : 'remove_default' '(' parameterReference ')' NEWLINE;
...
// composition
composition : compositionOperator ('(' argumentList ')')?':' NEWLINE INDENT
...
emitDirective :  'emit' eventName ('(' argumentList ')')? NEWLINE;

callDirective : 'call'  methodInvocation NEWLINE;

untilDirective : 'until' eventSpecification NEWLINE;

methodInvocation : postfixExp '(' (argumentList)? ')';

methodDeclaration : 'def' methodName '(' (argumentListSpecification)? ')' ('->' returnType)? methodImplementation NEWLINE;
